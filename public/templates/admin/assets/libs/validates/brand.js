document.addEventListener("DOMContentLoaded", function () {
    ...
    const brands = window.brands || []; // Định nghĩa sẵn trong Blade
    ...
    forms.forEach((form) => {
        form.addEventListener("submit", function (event) {
            ...
            const idBrand = form.querySelector("[name='id']").value;
            ...
            if (
                brands.some(
                    (brand) => brand.slug === slug && brand.id != idBrand
                )
            ) {
            ...
            if (name.value.trim().length > 40) {
                showError(name, "Tên thương hiệu quá dài");
                isValid = false;
            }
...
document.addEventListener("DOMContentLoaded", function () {
    document.querySelectorAll(".edit-item-btn").forEach((button) => {
        button.addEventListener("click", function () {
            ...
            document.getElementById("description-field-edit").value =
                description;
