(function ($) {
    ...
    $('.lms_table_active').DataTable({
        scrollX: true, // Thêm cuộn ngang
        responsive: true,
        autoWidth: false // Ngăn DataTables tự động thay đổi kích thước
    });
    if ($(".lms_table_active").length) {
        $(".lms_table_active").DataTable({
            bLengthChange: false,
            destroy: true, // Cho phép cập nhật bảng mà không làm mất dữ liệu
            language: {
            ...
            columnDefs: [
                {
                    targets: [2], // Ẩn cột thứ 3 (chứa mật khẩu chẳng hạn)
                    visible: false,
                },
            ],
            responsive: true,
    ...
    if ($(".lms_table_active2").length) {
        $(".lms_table_active2").DataTable({
            ...
            columnDefs: [
                {
                    targets: [1], // Ẩn cột thứ 2 nếu cần
                    visible: false,
                },
            ],
            responsive: true,
            searching: true, // Bật tìm kiếm
            info: true, // Hiện số dòng hiển thị
