            ...
            form.addEventListener('submit', function(event) {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        document.getElementById('product-id-modal').value = data.data.id;
                        document.getElementById('product-name-modal').textContent = data.data.name;
                        document.getElementById('product-quantity-modal').textContent = 'Còn ' + data.data.quantity + ' sản phẩm';    
                        const initialQuantity = data.data.quantity;
                        document.getElementById('product-image-modal').setAttribute('src', 'http://127.0.0.1:8000/storage/' + data.data.image);
                        ...
                        if (priceSaleProduct > 0 && priceSaleProduct < priceProduct) {
                            document.getElementById('product-price-modal').textContent = `${priceProduct.toLocaleString('vi-VN')} VND`;
                            document.getElementById('product-price-sale-modal').textContent = `${priceSaleProduct.toLocaleString('vi-VN')} VND`;
                        } else {
                            document.getElementById('product-price-sale-modal').textContent = `${priceProduct.toLocaleString('vi-VN')} VND`;
                        ...
                        function updateQuantityDisplay() {
                            ...
                            if (!activeColor && !activeSize) {
                                document.getElementById('product-quantity-modal').textContent = 'Còn ' + initialQuantity + ' sản phẩm';
                                if (priceSaleProduct > 0 && priceSaleProduct < priceProduct) {
                                    document.getElementById('product-price-modal').textContent = `${priceProduct.toLocaleString('vi-VN')} VND`;
                                    document.getElementById('product-price-sale-modal').textContent = `${priceSaleProduct.toLocaleString('vi-VN')} VND`;
                                } else {
                        ...
                        function updateSizeAvailability(selectedColorId) {
                            ...
                            sizeButtons.forEach(button => {
                                ...
                                const variant = data.data.variants.find(v => 
                                    v.color_id == selectedColorId && v.size_id == sizeId
                                );
                                
                                if (variant && variant.quantity > 0) {
                                    const priceVariant = Number(variant.price);
                                    const priceSaleVariant = Number(variant.price_sale);

                                    button.disabled = false;
                                    button.classList.remove('btn-disabled');
                                    if (button.classList.contains('active')) {
                                        document.getElementById('product-quantity-modal').textContent = 'Còn ' + variant.quantity + ' sản phẩm';
                                        if (priceSaleVariant > 0 && priceSaleVariant < priceVariant) {
                                            document.getElementById('product-price-modal').textContent = `${priceVariant.toLocaleString('vi-VN')} VND`;
                                            document.getElementById('product-price-sale-modal').textContent = `${priceSaleVariant.toLocaleString('vi-VN')} VND`;
                                        } else {
                                            document.getElementById('product-price-sale-modal').textContent = `${priceVariant.toLocaleString('vi-VN')} VND`;
                                            document.getElementById('product-price-modal').textContent = '';
                                        }
                                         document.getElementById('product-image-modal').setAttribute('src', 'http://127.0.0.1:8000/storage/' + variant.image);
                                    }
                                } else {
                            ...
                            if (!activeSize || activeSize.disabled) {
                                const firstAvailableSize = document.querySelector('.size-btn:not(.btn-disabled)');
                        ...
                        function updateColorAvailability(selectedSizeId) {
                            ...
                            colorButtons.forEach(button => {
                                const colorId = button.getAttribute('data-value');
                                const variant = data.data.variants.find(v => 
                                    v.color_id == colorId && v.size_id == selectedSizeId
                                );
                            ...
                            if (!activeColor || activeColor.disabled) {
                                const firstAvailableColor = document.querySelector('.color-btn:not(.btn-disabled)');
                            ...
                            button.addEventListener('click', function() {
                                if (!this.disabled) {
                                    ...
                                    if (!isActive) {
                                        ...
                                        const selectedColorId = this.getAttribute('data-value');
                                        colorForCart.value = selectedColorId;
                                        updateSizeAvailability(selectedColorId);
                                    } else {
                                        colorForCart.value = '';
                                        const sizeButtons = document.querySelectorAll('.size-btn');
                                        sizeButtons.forEach(button => {
                                        ...
                                        updateQuantityDisplay(); // Cập nhật số lượng khi bỏ chọn
                                    }
                                }
                            });
                        });
                        ...
                        sizeButtons.forEach(button => {
                            button.addEventListener('click', function() {
                                if (!this.disabled) {
                                    ...
                                    if (!isActive) {
                                        this.classList.add('active');
                                        const selectedSizeId = this.getAttribute('data-value');
                                        sizeForCart.value = selectedSizeId;
                                        updateColorAvailability(selectedSizeId);
                                    } else {
                                        sizeForCart.value = '';
                                        const colorButtons = document.querySelectorAll('.color-btn');
                                        colorButtons.forEach(button => {
